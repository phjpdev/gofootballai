...
const DIVIDER_POSITIONS = [68.6, 137.2, 205.8, 274.4];

function toPercent(px: number): string {
  ...
  return (
    <div
      className={cn(
        ...
        middleContent ? "gap-4 lg:gap-6" : "gap-8",
      )}
    >
      <p className="w-full text-center text-sm font-extrabold uppercase leading-5 tracking-[1.4px] text-orange-50 lg:text-base">
        等級 {clampedLevel}
      </p>

      <div className="relative h-9 w-full max-w-[343px] rounded-2xl bg-[#27272a] shadow-[0px_4px_4px_rgba(15,23,42,0.03),0px_8px_8px_rgba(15,23,42,0.02)] lg:max-w-3xl">
        <div
          className="absolute left-0 top-0 h-9 rounded-full bg-gradient-to-l from-[#65a30d] via-[51.5%] via-[#f59e0b] to-[#f43f5e] transition-[width] duration-500 ease-out"
          style={{ width: toPercent(fillWidth) }}
        />
        ...
        {LEVEL_POSITIONS.map((left) => (
          <div
            ...
            style={{ left: toPercent(left) }}
          />
        ))}

        {DIVIDER_POSITIONS.map((left) => (
          <div
            key={left}
            className="absolute top-1.5 h-6 w-px -translate-x-1/2"
            style={{ left: toPercent(left) }}
          >
            <Image
        ...
        <div
          className="absolute top-1/2 flex size-10 -translate-x-1/2 -translate-y-1/2 items-center justify-center rounded-full border-4 border-[#27272a] bg-[#65a30d] shadow-[0px_12px_8px_rgba(15,23,42,0.08),0px_4px_3px_rgba(15,23,42,0.03)] transition-[left] duration-500 ease-out"
          style={{ left: toPercent(thumbLeft) }}
        >
          <Image
